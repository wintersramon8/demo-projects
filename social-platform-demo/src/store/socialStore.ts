...
export const useSocialStore = create<SocialStore>((set, get) => ({
  ...
  toggleLike: async (postId) => {
    ...
    try {
      ...
      await get().loadPosts(); // Refresh to get accurate like status
    } catch (error: any) {
      // Rollback on error
      set((state) => ({
        posts: state.posts.map((p) =>
  ...
  addComment: async (postId, content) => {
    ...
    try {
      ...
      await get().loadPosts(); // Update comment count
      set({ loading: false });
    } catch (error: any) {
      set({ error: error.message, loading: false });
      throw error;
  ...
  getUserFollowers: (userId) => {
    return get().users.filter((u) => {
      // This would need to check connections, but for now return from users array
      return true; // Simplified - would need connection checking
    });
  },
